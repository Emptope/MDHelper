...
- 进程内轨迹格式支持取决于内置 MDAnalysis；较新 TPR 可能需要兼容的 GRO/PDB topology
  snapshot。TNG 是 GROMACS 轨迹格式，但当前
  MDAnalysis/PyTNG reader 不能可靠读取有效的 GROMACS TNG 输出，因此 MDHelper 0.1.0
  不支持 TNG。
- GROMACS 是可选工具。Auto 可在前序完整候选无法加载输入后选择已检测的完整 GROMACS
  流水线；显式 `gromacs` RDF/cumulative RDF 使用已安装的 `gmx trjconv` 和 `gmx rdf`，数值行为可能随
