...
第一壳层诊断修订版 2 不修改已完成的 RDF。至少需要三个有限数据点，Savitzky-Golay 使用最大 11 点的奇数窗口（受样本数一半限制，最小三点），只用于定位候选特征：
...
候选特征必须在半个滤波窗口内匹配原始曲线上的显著局部极值。取邻域内最高原始峰或最深原始谷的样本索引、半径和数值，平坦极值取中间样本，以排除滤波振铃和位置偏移。首个解析出的原始峰及其后的原始谷定义壳层。缺少任一特征时边界不可用，并产生警告，但已解析出的峰仍可报告。边界不改变 `r_max`。累计配位数按累计方法定义，取边界处或其后的首个样本。RDF 和累计曲线的数值方法不变。
