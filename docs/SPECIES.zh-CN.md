...
inspection summary 显示每条建议的 method、evidence、confidence、candidate roles、reason
和确认要求。CLI 用结构化的
`--roles '{LI: cation, SOL: solvent}'` 显式确认；GUI 显示同一建议，要求确认并允许编辑。
request parameter provenance 只保存选择来源及无法从最终角色恢复的逐 species 建议事实。
最终角色的唯一来源是 `request.species_roles`，result provenance 不再复制。已确认角色存入 project manifest，
`mdhelper project set-roles` 可替换该映射，不改变机器配置。

角色只为项目 metadata 和结果解释保留化学上下文。它绝不生成或替换
atom selection、选择 cutoff/grid/algorithm 或改变数值。`inspect` 会发布该 policy 和所有
允许角色的定义，使 CLI/TUI/GUI 共享契约。持久化请求证据不再重复全局候选角色、恒定确认
标记、可推导的 available/decision、最终角色和固定 charge tolerance。
